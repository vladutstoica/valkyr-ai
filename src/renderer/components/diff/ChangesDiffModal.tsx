...
import { fsRead, fsWriteFile } from '@/services/fsService';
...
export const ChangesDiffModal: React.FC<ChangesDiffModalProps> = ({
  ...
  useEffect(() => {
    ...
    const loadFileData = async () => {
      ...
      try {
        ...
        const diffRes = await getFileDiff({ taskPath: safeTaskPath, filePath });
  ...
  const handleSave = async () => {
    ...
    setSaveError(null);
    try {
      const res = await fsWriteFile(safeTaskPath, selected, modifiedDraft, true);
      if (!res?.success) {
