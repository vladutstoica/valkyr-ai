    ...
    <Collapsible open={isOpen} onOpenChange={setIsOpen} className="w-full">
      <CollapsibleTrigger asChild>
        <Button
          variant="ghost"
          size="sm"
          className="group/scripts text-muted-foreground hover:text-foreground flex w-full items-center justify-between px-2 py-1.5 text-xs font-medium"
        >
          <div className="flex items-center gap-2">
            <Terminal className="h-3.5 w-3.5" />
            <span>Scripts</span>
            {scripts.length > 0 && (
              <span className="text-muted-foreground text-[10px]">({scripts.length})</span>
            )}
          </div>
          {isOpen ? (
      ...
      <CollapsibleContent>
        <div className="px-2 pb-2">
          {isLoading && (
            <div className="text-muted-foreground py-2 text-center text-xs">Loading scripts...</div>
          )}

          {error && <div className="text-destructive py-2 text-center text-xs">{error}</div>}
              ...
              <div className="space-y-0.5">
                {scripts.map((script) => {
                  ...
                  return (
                    <div
                      key={script.name}
                      className={`group/script hover:bg-accent flex items-center justify-between rounded px-2 py-1 ${isRunning ? 'cursor-pointer' : ''}`}
                      onClick={() => {
                        if (isRunning) {
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent side="right" className="max-w-xs">
                              <div className="space-y-1">
                                ...
                                <code className="block text-[10px] whitespace-pre-wrap">
                                  {script.command}
                                </code>
                              </div>
                            </TooltipContent>
